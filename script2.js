...
async function getStockDetails(stock) {
    const apiUrl = `https://stocks3.onrender.com/api/stocks/getstocksprofiledata`;
    const response = await fetch(apiUrl);
    const data = await response.json();
    return {
        bookValue: data.bookValue,
        profit: data.profit,
        summary: await getStockSummary(stock) // Fetch summary separately
    };
}

async function getStockSummary(stock) {
    const summaryUrl = `https://stocks3.onrender.com/api/stocks/getstockstatsdata`;
